...
extern uint8_t si4734_rd_buf[9];          //buffer for holding data recieved from the si4734
...
extern volatile uint8_t STC_interrupt;     //flag to indicate when tune or seek is done
...
ISR(TIMER0_COMP_vect){
	...
	if (chk_buttons(2)){
		mode ^= 1<<2; //toggle the bit on the bar graph that corresponds to the button
		radio_flag = 1; //set radio flag
	...
	pinB1 = ((encoder_data & 0x02) == 0) ? 0 : 1; //sample pinB from encoder 1 //encoder1 state machine
	switch (encoder1){
		case IDLE:
...
//************************************************************************
//			radio_init
// Performs the radio reset sequence
void radio_init(){
